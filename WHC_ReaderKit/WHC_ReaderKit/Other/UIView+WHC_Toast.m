...
@interface WHC_ToastLabel : UILabel

@end

@implementation WHC_ToastLabel
...
- (UILabel*)createContentLabWithMessage:(NSString*)msg during:(NSTimeInterval)during postion:(WHC_TOAST_POSTION)postion type:(WHC_TOAST_TYPE)type{
    for (WHC_ToastLabel * tempLable in self.subviews) {
        if ([tempLable isKindOfClass:[WHC_ToastLabel class]]) {
            [tempLable removeFromSuperview];
        }
    }
    self.userInteractionEnabled = NO;
    CGSize      screenSize = self.frame.size;//[UIScreen mainScreen].bounds.size;
...
#pragma clang diagnostic pop
    ...
    WHC_ToastLabel * contentLab = [[WHC_ToastLabel alloc]initWithFrame:CGRectMake((screenSize.width - contentLabWidth) / 2.0,contentLabY,contentLabWidth,multiple * msgSize.height + pading)];
