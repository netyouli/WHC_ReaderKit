...
#define KWHC_Iphone4ScreenWidth (IS_IPAD ? 1024.0 : 320.0)      //iphone4屏幕宽度
#define KWHC_Iphone4ScreenHeight (IS_IPAD ? 768.0 : 480.0)     //iphone4屏幕高度
...
- (void)autoHoriAdapterWithLabelSacleType:(WHCAutoAdpterLabelType)scaleType{
    ...
    for(NSInteger s = 0; s < rowViewArr.count; s++){
            ...
            if(![view isKindOfClass:[UIImageView class]]){
                switch (scaleType) {
                    ...
                    case LabelRatioSacle:
                        rc.origin.x   *= (CGRectGetWidth([UIScreen mainScreen].bounds) / 320.0);
                        rc.size.width *= (CGRectGetWidth([UIScreen mainScreen].bounds) / 320.0);
                        break;
                    case LabelStretchSacle:{
                        if(CGRectGetWidth(view.superview.frame) == [self screenWidth]) {
                            ...
                            if ([view isKindOfClass:[UILabel class]] ||
                                ...
                                rc.size.width = CGRectGetWidth(self.frame) - minXPading;
                            }else {
                                rc.size.width = CGRectGetWidth(self.frame) - minXPading * 2;
                            }
                        }
                    }
                        break;
        ...
        CGFloat   offsetx = 0;
        if (subRowViewArr.count > 1) {
            offsetx = CGRectGetMinX(((UIView *)subRowViewArr[0]).frame);
        }
        CGFloat   differ = CGRectGetWidth(self.frame) - CGRectGetMaxX(v.frame) - offsetx;
        if(differ < 0.0){
        ...
        CGFloat   avgDiffer = 0;
        if(variableCount != 0){
            avgDiffer = differ / (CGFloat)variableCount;
        }
